...
for ℓ₁ ∈ 0:4, ℓ₂ ∈ 0:4, k ∈ 1:(nmax+3)
    I∞[:,ℓ₁+1,ℓ₂+1,k] = [ quadgk(τ -> exp(-sin(π_(A[i+1]))*τ) *
             max(τ^(k*A[i]),τ^(k*A[i+1])) * τ^(ℓ₁) *
             abs(log(τ))^(ℓ₂),
...
## Oscillatory bound coefficients

osc = zeros(4)

osc[1] = quadgk( t -> abs(log(t)*t - log(t) - 1)*exp(-t) , 0, Inf, rtol=integral_tolerance )[1]
osc[2] = quadgk( t -> abs(log(t))*abs(log(t)*t^2 - 3*log(t)*t - 2*t + log(t) + 2)*exp(-t) , 0, Inf, rtol=integral_tolerance )[1]
osc[3] = quadgk( t -> abs(log(t))^2*abs(log(t)*t^3 - 6*log(t)*t^2 - 3*t^2 + 7*log(t)*t + 9*t - log(t) - 3)*exp(-t) , 0, Inf, rtol=integral_tolerance )[1]
osc[4] = quadgk( t -> abs(log(t))^3*abs(log(t)*t^4 - 10*log(t)*t^3 - 4*t^3 + 25*log(t)*t^2 + 24*t^2 - 15*log(t)*t - 28*t + log(t) + 4)*exp(-t) , 0, Inf, rtol=integral_tolerance )[1]
